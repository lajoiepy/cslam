...
std::vector<cslam_common_interfaces::msg::PoseGraphValue> gtsam_values_to_msg(const gtsam::Values::shared_ptr values)
{
    ...
    for (const auto key_value : *values) {
        ...
        pose_msg.key.robot_id = key.label();
...
std::vector<cslam_common_interfaces::msg::PoseGraphEdge> gtsam_factors_to_msg(const gtsam::NonlinearFactorGraph::shared_ptr factors)
{
    ...
    for (const auto &factor_ : *factors) {
        ...
        if (factor) {
            ...
            edge_msg.key_from.robot_id = key_from.label();
            edge_msg.key_from.keyframe_id = key_from.index();
            edge_msg.key_to.robot_id = key_to.label();
...
gtsam::Pose3 transform_msg_to_pose3(const geometry_msgs::msg::Transform& msg){
  ...
  return gtsam::Pose3(rotation, {msg.translation.x, msg.translation.y, msg.translation.z});
}

gtsam::Pose3 pose_msg_to_gtsam(const geometry_msgs::msg::Pose& pose){
  gtsam::Rot3 rotation(pose.orientation.w, pose.orientation.x, pose.orientation.y, pose.orientation.z);
  return gtsam::Pose3(rotation, {pose.position.x, pose.position.y, pose.position.z});
}
...
gtsam::Values::shared_ptr values_msg_to_gtsam(const std::vector<cslam_common_interfaces::msg::PoseGraphValue>& msg){
    auto values = boost::make_shared<gtsam::Values>();
    for (auto v : msg){
        gtsam::Pose3 pose = pose_msg_to_gtsam(v.pose);
        gtsam::LabeledSymbol symbol(GRAPH_LABEL, v.key.robot_id, v.key.keyframe_id);
        values->insert(symbol, pose);
    }
    return values;
}

gtsam::NonlinearFactorGraph::shared_ptr edges_msg_to_gtsam(const std::vector<cslam_common_interfaces::msg::PoseGraphEdge>& msg){
    auto graph = boost::make_shared<gtsam::NonlinearFactorGraph>();
    for (auto e: msg){
        gtsam::Pose3 pose = pose_msg_to_gtsam(e.measurement);
        gtsam::LabeledSymbol symbol_from(GRAPH_LABEL, e.key_from.robot_id, e.key_from.keyframe_id);
        gtsam::LabeledSymbol symbol_to(GRAPH_LABEL, e.key_to.robot_id, e.key_to.keyframe_id);

        Eigen::VectorXd sigmas(6);
        sigmas << e.noise_std[0], e.noise_std[1], e.noise_std[2], 
                    e.noise_std[3], e.noise_std[4], e.noise_std[5];
        auto noise_model = gtsam::noiseModel::Diagonal::Sigmas(sigmas);

        gtsam::BetweenFactor<gtsam::Pose3> factor(symbol_from, symbol_to, pose, noise_model);
